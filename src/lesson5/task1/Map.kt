...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((key, value) in mapA) {
        if (mapB.containsKey(key) && value != mapB[key]) ans[key] = value + ", " + mapB[key]!!
        else ans[key] = mapA[key]!!
    }
    for ((key, value) in mapB) {
        if (!ans.containsKey(key)) ans[key] = value
    }
    return ans
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        ...
        if ((number - element) in map && i != map[(number - element)]!!) {
            return Pair(i, map[(number - element)]!!)
        }
    }
    return Pair(-1, -1)
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    val res = mutableSetOf<String>()
    val bp = MutableList(treasures.size + 1) { MutableList(capacity + 1) { 0 } }
    for (i in treasures.keys.indices) {
        for (j in 0..capacity) {
            if (weight[i] > j) bp[i + 1][j] = bp[i][j]
            else bp[i + 1][j] = maxOf(bp[i][j], bp[i][j - weight[i]] + value[i])
        }
    }
    var curCap = capacity
    for (i in treasures.size - 1 downTo 0) {
        if (bp[i + 1][curCap] != bp[i][curCap]) {
            curCap -= weight[i]
            res.add(treasures.keys.toList()[i])
        }
    }
    return res
}
